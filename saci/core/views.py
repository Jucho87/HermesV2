...
from .models import (
    Categoria, Marca, UnidadMedida, Producto, ProductoDetalle,
    ListaCompra, TransaccionDetalle
)
from .serializers import (
    CategoriaSerializer, MarcaSerializer, UnidadMedidaSerializer,
    ProductoSerializer, ProductoDetalleSerializer, ListaCompraSerializer,
...
class ProductoDetalleViewSet(viewsets.ModelViewSet):
    queryset = ProductoDetalle.objects.select_related('producto__categoria', 'marca').all()
...
class ListaCompraViewSet(viewsets.ModelViewSet):
        ...
        if serializer.is_valid():
            ...
            try:
                with transaction.atomic():
                    ...
                    lista.save()
                return Response({'status': 'compra registrada exitosamente'}, status=status.HTTP_200_OK)
    ...
    def reporte_comparativo(self, request, pk=None):
        ...
        if not lista.fecha_ejecucion:
            return Response({'error': 'El reporte solo puede generarse para listas de compra ejecutadas.'}, status=status.HTTP_400_BAD_REQUEST)
        total_estimado = lista.total_estimado
        total_real = lista.total_real
        diferencia_absoluta = total_real - total_estimado
        diferencia_porcentual = (diferencia_absoluta / total_estimado * 100) if total_estimado > 0 else 0
        resumen = {
        ...
        transacciones_mapeadas = {t.item_lista_id: t for t in transacciones if t.item_lista_id}
        detalle_items = []
        for item_planeado in items_planeados:
            detalle = {
                'producto': str(item_planeado.producto_detalle),
                'planeado': {'cantidad': item_planeado.cantidad_planeada, 'precio_unitario': item_planeado.precio_estimado_unitario, 'subtotal': item_planeado.subtotal_estimado},
                'real': None
            }
            transaccion_real = transacciones_mapeadas.get(item_planeado.id)
            if transaccion_real:
                detalle['real'] = {'cantidad': transaccion_real.cantidad_real, 'precio_unitario': transaccion_real.precio_unitario_real, 'subtotal': transaccion_real.subtotal_real}
...
class ReportesAPIView(APIView):
    def _get_gasto_agrupado(self, group_by_field, group_by_name):
        queryset = TransaccionDetalle.objects.values(group_by_field).annotate(
            total_gastado=Sum(F('cantidad_real') * F('precio_unitario_real'))
    ...
    def get(self, request, *args, **kwargs):
        ...
        if reporte_tipo == 'gasto_por_categoria':
            return self._get_gasto_agrupado('producto_detalle__producto__categoria__nombre', 'categoria')
        elif reporte_tipo == 'gasto_por_marca':
            return self._get_gasto_agrupado('producto_detalle__marca__nombre', 'marca')
        return Response({'error': 'Tipo de reporte no especificado o inválido.'}, status=status.HTTP_400_BAD_REQUEST)

class TendenciaPreciosAPIView(APIView):
    def get(self, request, producto_detalle_id, *args, **kwargs):
        queryset = TransaccionDetalle.objects.filter(
            producto_detalle_id=producto_detalle_id
        ).order_by('lista_compra__fecha_ejecucion').values(
            'lista_compra__fecha_ejecucion',
